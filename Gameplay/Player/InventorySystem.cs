...
public partial class InventorySystem : Panel
{
    ...
    MongoInventoryItem _mouseHoverItem = new();
    PackedScene _hoverUiScene = ResourceLoader.Load<PackedScene>("res://Gameplay/Player/Ui/HoverUi.tscn");
    HoverUi _hoverUiInstance;

    PackedScene _upgradeUiScene = ResourceLoader.Load<PackedScene>("res://Gameplay/Player/Ui/BlacksmithUpgradeUi.tscn");
    BlacksmithUpgradeUi _upgradeUiInstance;
    public override void _Ready()
    {
        ...
        _currencyLabel = GetNode("CurrencyPanel/CurrencyLabel") as Label;
        ...
        _hoverUiInstance = _hoverUiScene.Instantiate() as HoverUi;
        _hoverUiInstance.Visible = false;
        GetParent().CallDeferred("add_child", _hoverUiInstance);
    }
    public void ToggleInventory()
    {
        Visible = !Visible;
    }
    void FindItemAtMousePos(Vector2 localMousePos, (int x, int y) mouseTilePos, int tileIndex)
    {
        if (tileIndex >= Tiles.Count || tileIndex < 0) { 
            _mouseHoverItem = new MongoInventoryItem(); ;
            return;
        }
        ...
        if(_mouseHoverItem != item)
        {
            ...
            GD.Print("MOUSE OVER: " + _mouseHoverItem.ItemType.ToString());
        }
    }
    public override void _Process(double delta)
    {
        var mouseIsInInv = Utility.MouseIsInControl(this);
        ...
        if (mouseIsInInv)
        {
            FindItemAtMousePos(localMousePos, mouseTilePos, tileIndex);
            SetupHoverUi();
        }
        else
        {
        ...
        {
            _currentlyMoving = _mouseHoverItem.Id;
        }
        ...
        if (Input.IsActionJustReleased("MouseLeft"))
        {
            if (_currentlyMoving != Guid.Empty)
            {
                ...
                if (!mouseIsInInv)
                {
                    if (HandleItemDroppedOnNpc(item))
                    {
                        invItem.Position = new Vector2(item.TilePosTopLeftX * TILE_PIXEL_SIZE, item.TilePosTopLeftY * TILE_PIXEL_SIZE);
                    }
                    else
                    {
        ...
        {
            mouseTilePos = Utility.LocalMousePosToTilePos(localMousePos, TILE_PIXEL_SIZE);
            tileIndex = Utility.GridXAndYPosToIndex(mouseTilePos.Item1, mouseTilePos.Item2, TILE_WIDTH);
    ...
    bool HandleItemDroppedOnNpc(MongoInventoryItem movingItem)
    {
        var ge = Utility.CheckIfMouseOnGameEntity(this);
        if(ge is Npc npc)
        {
            if(ge is Blacksmith blacksmith)
            {
                if(!IsInstanceValid (_upgradeUiInstance))
                {
                    _upgradeUiInstance = _upgradeUiScene.Instantiate() as BlacksmithUpgradeUi;
                    _upgradeUiInstance.Initialize(movingItem);
                    _upgradeUiInstance.Position = Vector2.Zero;
                    GetParent().CallDeferred("add_child", _upgradeUiInstance);
                }
            }
            return true;
        }
    ...
    void SetupHoverUi()
    {
        var parent = _hoverUiInstance.GetParent() as Control;
        var parentMousePos = parent.GetLocalMousePosition();
        var width = _hoverUiInstance.Size.X;
        _hoverUiInstance.Visible = false;
        if(_mouseHoverItem.Id != Guid.Empty)
        {
            _hoverUiInstance.SetData(_mouseHoverItem.Name, _mouseHoverItem.DefaultStats.ToList());
            _hoverUiInstance.Visible = true;
            _hoverUiInstance.Position = parentMousePos - new Vector2(width/2,-20);
        }
    }
    ...
    public void HandleInventoryUpdate(List<MongoInventoryItem> items, long currencyAmount)
    {
        CurrencyAmount = currencyAmount;
        _currencyLabel.Text = "Gold: " + CurrencyAmount;
        ...
        {
            var newItem = new MongoInventoryItem(items[i]);

            if(newItem.ItemType == ItemType.Currency)
            {
                CurrencyAmount += newItem.ItemValue;
            }
            else
            {
                ...
                var invScene = ResourceLoader.Load<PackedScene>(ItemInfo.ItemTypeToScenePath(newItem.ItemType, ItemInfo.SceneType.InventoryScene));
                var sceneInstance = invScene.Instantiate() as Control;
                AddChild(sceneInstance);
                sceneInstance.Position = new Vector2(newItem.TilePosTopLeftX * TILE_PIXEL_SIZE, newItem.TilePosTopLeftY * TILE_PIXEL_SIZE);
                InventoryUiItems.Add(newItem.Id, sceneInstance);
            }
        }
    }

    public void UpdateItem(MongoInventoryItem update)
    {
    ...
    void UnoccupyTiles(Guid item)
    {
        foreach(var tile in Tiles)
        {
            if(tile.OccupiedBy == item)
            {
                tile.OccupiedBy = Guid.Empty;
